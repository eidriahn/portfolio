...
export class AttrI18n {
  ...
  public async init(): Promise<void> {
    ...
    this.currentLanguage = this.options.defaultLang;
    if (!lang) {
      this.printMsg(
        "No language is set, default language will be used.",
        AttrI18nDebugLevel.WARN
      );
    ...
    this.printMsg(
      `${elements.length} elements were translated to ${lang}.`,
      AttrI18nDebugLevel.DEBUG
    );
  }
  ...
  onChange(language: string): void {
    ...
    if (language !== currentLanguage) {
      this.printMsg(
        `Switching to ${language} from ${currentLanguage}`,
        AttrI18nDebugLevel.INFO
      );
      ...
      this.printMsg(
        `Selected language is the same as current language`,
        AttrI18nDebugLevel.DEBUG
      );
    }
  }
  ...
  private attachOnChangeToi18nToggler(id: string): void {
    ...
    if (langToggler) {
      ...
      this.printMsg(
        `No language selector with id "${id}" found.`,
        AttrI18nDebugLevel.WARN
      );
    }
  }
  ...
  private translate(
    ...
    for (const element of elements) {
      ...
      const text = keys.reduce(
        (obj: Record<string, string>, i: string) =>
          obj[i] as unknown as Record<string, string>,
        translation
...
export const i18n = new AttrI18n(languages, {
  attr: "i18n-key",
  ...
  onLanguageChange: (lang) => {
    ...
    contributionsCount.innerHTML = `${totalContributions} ${
